...
{
    // 帧同步状态 信息显示 
	public class FrameSyncStats : MonoBehaviour
	{
        // 状态UI类
		...
		private Dictionary<string, FrameSyncStats.StatsUI> statsUI = new Dictionary<string, FrameSyncStats.StatsUI>();
		...
		public void Start()
		{
			...
			this.statsUI["ping"] = new FrameSyncStats.StatsUI("Ping", Color.black, 500f, true);
			this.statsUI["missed_frames"] = new FrameSyncStats.StatsUI("Missed Frames", FrameSyncStats.COLOR_YELLOW, 100f, false);
			this.statsUI["rollback"] = new FrameSyncStats.StatsUI("Rollbacks", Color.magenta, 100f, false);
			this.statsUI["simulated_frames"] = new FrameSyncStats.StatsUI("Sim. Frames", Color.blue, 100f, false);
			this.statsUI["panic"] = new FrameSyncStats.StatsUI("Panic", Color.red, 100f, false);
			this.statsUI["players"] = new FrameSyncStats.StatsUI("Active Players", Color.black, 100f, false);
		...
		private void DrawOfflineMode(ref int posBaseY)
		{
			...
			{
                GUI.skin.label.alignment = TextAnchor.MiddleLeft;
				GUI.contentColor = this.lockstep.checksumOk ? FrameSyncStats.COLOR_GREEN : Color.red;
				this.reusableRect.Set((float)(this.width + this.marginRight), (float)posBaseY, (float)this.globalTextWidth, 20f);
